...
if __name__ == "__main__":
    ...
    #desData = sc.textFile('Description.csv').mapPartitions(lambda x:reader(x))
    desData = sc.textFile(sys.argv[2], 1).mapPartitions(lambda x:reader(x))
    desheader = desData.first()
    deslines = desData.filter(lambda line: line != desheader)
    description=dict(deslines.map(lambda x: (x[0], x[1])).collect())
    ...
    for i in range(num_features):
        ### format of the map: Key, base_type, semantic_type, Null/Invalid/Valid label, detailed description ###
        result = lines.map(lambda x : x[i]).map(lambda x : (x, (Checker(x).base_type(), Checker(x).semantic_type(), Checker(x).label(),description[header[i]])))
